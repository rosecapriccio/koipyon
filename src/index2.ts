...
export class MyScene2 extends Phaser.Scene {
  constructor() {
  ...
  preload() {
    ...
    this.load.spritesheet("stageall2", "assets/groundsall2.png", {
      frameWidth: 50,
      frameHeight: 25,
    });
  ...
  create() {
    ...
    this.anims.create({
      key: "stage12",
      frames: this.anims.generateFrameNumbers("stageall2", {
        start: 0,
        end: 0,
      }),
      frameRate: 5,
    ...
    this._stage.children.iterate((s, index) => {
      ...
      stage.anims.play(
        "stage" + this.stageinfo[index].type.toString() + "2",
        true
      );
      return true;
    });
